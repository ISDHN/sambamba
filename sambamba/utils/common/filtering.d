...
{
    return reads.zip(f.repeat()).filter!q{a[1].accepts(a[0])}
        .map!"a[0]"();
}
...
final class IntegerFieldFilter(string op) : Filter
{
    ...
    bool accepts(ref BamRead a)
    {
        ...
        {
        case "ref_id":
            mixin("return a.ref_id " ~ op ~ "_value;");
        case "position":
            mixin("return a.position " ~ op ~ "_value;");
        case "mapping_quality":
            mixin("return a.mapping_quality " ~ op ~ "_value;");
        case "avg_base_quality":
            mixin("return a.avg_base_quality " ~ op ~ "_value;");
        case "sequence_length":
            mixin("return a.sequence_length " ~ op ~ "_value;");
        case "mate_ref_id":
            mixin("return a.mate_ref_id " ~ op ~ "_value;");
        case "mate_position":
...
final class StringFieldFilter(string op) : Filter
{
    ...
    bool accepts(ref BamRead a)
    {
        ...
        {
        case "read_name":
            mixin("return a.name " ~ op ~ " _value;");
        case "sequence":
            mixin("return cmp(a.sequence, _value) " ~ op ~ " 0;");
        case "cigar":
            mixin("return a.cigarString() " ~ op ~ " _value;");
        case "strand":
            mixin("return a.strand " ~ op ~ " _value[0];"); // FIXME: a separate filter would be cleaner
        case "ref_name":
            mixin("return a.ref_name " ~ op ~ " _value;");
        case "mate_ref_name":
            mixin("return a.mate_ref_name " ~ op ~ " _value;");
        default:
            throw new Exception("unknown string field '" ~ _fieldname ~ "'");
        }
    }
}
...
final class RegexpFieldFilter : Filter
{
    ...
    bool accepts(ref BamRead a)
    {
        ...
        {
        case "read_name":
            return !match(a.name, cast() _pattern).empty;
        case "ref_name":
            return !match(a.ref_name, cast() _pattern).empty;
        case "mate_ref_name":
            return !match(a.mate_ref_name, cast() _pattern).empty;
        case "sequence":
            return !match(to!string(a.sequence), cast() _pattern).empty;
        case "cigar":
            return !match(a.cigarString(), cast() _pattern).empty;
        default:
            throw new Exception("unknown string field '" ~ _fieldname ~ "'");
        }
    }
