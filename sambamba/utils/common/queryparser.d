    ...
    this(string op, LeftNodeType node, RightNodeType valuenode)
    {
        ...
        else
        {
            ...
            case "==":
                ...
                break;
            default:
                assert(0); // must be catched before calling constructor
            }
        }
    }
...
final class FlagConditionNode : ConditionNode
{
    ...
    this(in string flagname)
    {
        ...
        {
        case "paired":
            condition = new FlagFilter!"is_paired"();
            break;
        case "proper_pair":
            condition = new FlagFilter!"proper_pair"();
            break;
        case "unmapped":
            condition = new FlagFilter!"is_unmapped"();
            break;
        case "mate_is_unmapped":
            condition = new FlagFilter!"mate_is_unmapped"();
            break;
        case "reverse_strand":
            condition = new FlagFilter!"is_reverse_strand"();
            break;
        case "mate_is_reverse_strand":
            condition = new FlagFilter!"mate_is_reverse_strand"();
            break;
        case "first_of_pair":
            condition = new FlagFilter!"is_first_of_pair"();
            break;
        case "second_of_pair":
            condition = new FlagFilter!"is_second_of_pair"();
            break;
        case "secondary_alignment":
            condition = new FlagFilter!"is_secondary_alignment"();
            break;
        case "failed_quality_control":
            condition = new FlagFilter!"failed_quality_control"();
            break;
        case "duplicate":
            condition = new FlagFilter!"is_duplicate"();
            break;
        case "supplementary":
            condition = new FlagFilter!"is_supplementary"();
            break;
        case "chimeric":
            condition = new ChimericFilter();
            break;
        default:
...
final class QueryGrammar : Grammar!Node
{
    this()
    {
        ...
        addSymbolToDict("null").setParser((in string str) {
            return cast(Node) new NullValueNode();
        });
        ...
        auto flagnames = [
            "paired", "proper_pair", "unmapped", "mate_is_unmapped",
        ...
        addSymbolToDict("(flag condition)", 0)
            ...
            .setParser((in string str) {
                return cast(Node) new FlagConditionNode(str);
            });

        auto integer_fields = [
            "ref_id", "position", "mapping_quality", "avg_base_quality",
        ...
        addSymbolToDict("(integer field)", 0)
            ...
            .setParser((in string str) {
                return cast(Node) new IntegerFieldNode(str);
            });

        auto string_fields = [
            "read_name", "sequence", "cigar", "strand", "ref_name",
            "mate_ref_name"
        ];
        ...
        addSymbolToDict("(string field)", 0)
            ...
            .setParser((in string str) {
                return cast(Node) new StringFieldNode(str);
            });
            ...
            return (Node a, Node b) {
                ...
                if (integer !is null)
                {
                    ...
                    else
                    {
                        ...
                        {
                            throw new Exception(
                                "expected tag or integer field name instead of '" ~
                                    a.toString() ~ "'");
                        }
                ...
                else if (str !is null)
                {
                    ...
                    else
                    {
                        ...
                        {
                            throw new Exception(
                                "expected tag or string field name instead of '" ~
                                    a.toString() ~ "'");
                        }
        ...
        addSymbolToDict("(regexp)", 0)
            .setScanner((in string str, size_t pos) {
                ...
                while (i < str.length)
                {
                    if (std.ascii.isWhite(str[i]) || str[i] == ')')
                        break;
        ...
        infix("and", 80, (Node a, Node b) {
            return cast(Node) new AndConditionNode(a, b);
        });
        infix("or", 60, (Node a, Node b) {
            return cast(Node) new OrConditionNode(a, b);
        });
...
unittest
{
    ...
    assert(grammar.parse("unmapped and mate_is_unmapped")
            .toString() == "(unmapped) and (mate_is_unmapped)");
